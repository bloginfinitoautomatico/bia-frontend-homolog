...
export function UserManagement({ onUpdateUser }: UserManagementProps) {
  ...
  const loadUsers = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.success(`${data.users.length} usuários carregados com sucesso`);
      } else {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Editor de Usuário - Placeholder */}
      {showUserEditor && selectedUser && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md">
            <h3 className="font-poppins text-lg font-bold mb-4">Editor de Usuário</h3>
            ...
            <p className="font-montserrat text-sm text-gray-500 mb-4">
              O editor de usuário será implementado em breve.
            </p>
            <Button
              onClick={() => {
              ...
              className="bg-[#8B5FBF] text-white hover:bg-purple-700"
            >
              Fechar
            </Button>
          </div>
